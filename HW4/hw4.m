...
for i = 1:wingCount
    ...
    fprintf('AR = %d\n', AR(i))
    fprintf('cLAlpha = %.3f\n', cLAlpha(i))
    fprintf('xCp as percentage of MAC = %.1f%%\n', ...
            xCpMacPercentage(i)*100)
    fprintf('xCp as percentage of (cR - cT) = %.1f%%\n', ...
...
legend('Finite wing theory', 'Slender wing theory', ...
...
plot([min(AR), max(AR)], finite.xCpMacPercentage*[1, 1], ...
    'LineWidth', lw, 'Color', colors.blue)
fill([6, 6, 10, 10], [0, 6, 6, 0], colors.blue, ...
     'LineStyle', 'none', 'FaceAlpha', 0.1)
plot(AR, xCpMacPercentage, 'x', ...
     'LineWidth', lw, 'MarkerSize', msX, 'Color', colors.red)
xticks(AR)
ylim([0.1, 0.4])
legend('Location according to finite wing theory', ...
       'Finite wing theory region', '$VLM$ location', ...
       'Location', 'Southeast');
hold off
...
plot([min(AR), max(AR)], slender.xCpCStarPercentage*[1, 1], ...
    'LineWidth', lw, 'Color', colors.blue)
fill([1, 1, 1.5, 1.5], [0, 6, 6, 0], colors.blue, ...
     'LineStyle', 'none', 'FaceAlpha', 0.1)
...
ylim([0.5, 0.8])
legend('Location according to finite wing theory', ...
       'Slender wing theory region', '$VLM$ location', ...
       'Location', 'Southeast');
hold off
