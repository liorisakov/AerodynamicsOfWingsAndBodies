classdef FlatWing < handle
    ...
    methods
        function obj = FlatWing(chordLeadingEdges, chordLengths, N)
            ...
            obj.xTileCount = obj.N(1);
            obj.yTileCount = 2*obj.N(2);
        end
        
        
        function CalculateGammaValues(obj, uoo, alpha)
            % initialization
            tileCount = obj.xTileCount*obj.yTileCount;
            A = zeros(tileCount/2, tileCount/2);

            % sum downwash at every control point on
            % positive y half of wing to build matrix A
            k = 1;
            for i = 1:obj.xTileCount
                for j = 1:obj.yTileCount/2
                    % current symmetric tile pair,
                    % at positive and negative y
                    yPosTile = obj.tiles{i,j};
                    yNegTile = obj.tiles{i,j+obj.yTileCount/2};

                    % contributions of both symmetric horseshoes
                    % at current control point
                    yPosDownwash = yPosTile.Downwash(obj.controlPointsX, ...
                                                     obj.controlPointsY, ...
                                                     'solve for gamma').';
                    yNegDownwash = yNegTile.Downwash(obj.controlPointsX, ...
                                                     obj.controlPointsY, ...
                                                     'solve for gamma').';
            ...
            obj.gammaValues = A\b;
            obj.gammaValues = reshape(obj.gammaValues, ...
                                      obj.yTileCount/2, obj.xTileCount).';
            obj.gammaValues = [obj.gammaValues, obj.gammaValues];

            for i = 1:obj.xTileCount
                for j = 1:obj.yTileCount
                    currentTile = obj.tiles{i,j};
                    currentTile.horseshoe.gamma = obj.gammaValues(i,j);
                end
            end
        end
        ...
        function [cl, cmApex] = AeroCoeffsAtY(obj, y, uoo, varargin)
            ...
            tilesAlongY = cell(obj.xTileCount, length(y));
            for k = 1:length(y)
                for j = 1:obj.yTileCount
            ...
            if ~returnSeparateXValues
                ...
                [gammaValues, gammaXoValues] = deal(zeros(obj.xTileCount, length(y)));
            end
            for k = 1:length(y)
                for i = 1:obj.xTileCount
                    ...
                    horseshoeMiddleX = currentHorseshoe.middle(1);
        ...
        function [cL, cMApex] = AeroCoeffs(obj, uoo)
            gamma = obj.gammaValues;
            deltaY = obj.tileSpans;
            xo = obj.horseshoeMidpointsX;
            
            sumGammaDeltaY = sum(gamma.*deltaY, 'all');
            sumGammaDeltaYXo = sum(gamma.*deltaY.*xo, 'all');
        ...
        function cT = LeadingEdgeSuction(obj, uoo, alpha)
            % sum downwash at horseshoe midpoint
            downwash = 0;
            for i = 1:obj.xTileCount
                for j = 1:obj.yTileCount
                    currentTile = obj.tiles{i,j};
                    ...
                    currentDownwash = ...
                        currentTile.Downwash(obj.horseshoeMidpointsX, ...
                                             obj.horseshoeMidpointsY, ...
                                             'trailing only');
                    downwash = downwash + currentDownwash;
                end
            end
            
            cT = 2/(uoo^2*obj.area) * ...
                 sum((uoo*alpha + downwash) .* ...
                      obj.gammaValues.*obj.tileSpans, 'all');
        end
        ...
        function handles = PlotSelf(obj, varargin)
            ...
            outlineY = [obj.chordLeadingEdges(:,2).', ...
             ...
             if ~noFill
                 fill3(outlineX + origin(1), ...
                       outlineY + origin(2), ...
                       zOffset*ones(size(outlineX)), ...
                      color, 'LineStyle', 'None', 'FaceAlpha', 0.2)
             end
            ...
            handles.outline = plot3(outlineX + origin(1), ...
                                    outlineY + origin(2), ...
                                    zOffset*ones(size(outlineX)), ...
                                    'LineWidth', lw, 'Color', color);
            if zOffset ~= 0
                plot3(obj.chordLeadingEdges(1,1)*[1, 1], ...
                      obj.chordLeadingEdges(1,2)*[1, 1], ...
                      [0, zOffset], '--k')
                for i = 2:length(obj.chordLengths)
                    plot3(obj.chordLeadingEdges(i,1)*[1, 1], ...
                          obj.chordLeadingEdges(i,2)*[1, 1], ...
                          [0, zOffset], '--k')
                    plot3(obj.chordLeadingEdges(i,1)*[1, 1], ...
                          - obj.chordLeadingEdges(i,2)*[1, 1], ...
                          [0, zOffset], '--k')
                      
                    plot3((obj.chordLeadingEdges(i,1) + obj.chordLengths(i))*[1, 1], ...
                          obj.chordLeadingEdges(i,2)*[1, 1], ...
                          [0, zOffset], '--k')
                    plot3((obj.chordLeadingEdges(i,1) + obj.chordLengths(i))*[1, 1], ...
                          - obj.chordLeadingEdges(i,2)*[1, 1], ...
            ...
            if plotMac
                ...
                handles.mac = plot3([obj.macLeadingEdgeX, macTrailingEdgeX], ...
                                    obj.macY*[1, 1], ...
                                    zoffset*[1, 1], ...
                                    'LineWidth', lw, 'Color', colors.red);
                handles.quarterMac = plot3(macQuarterX, ...
                                           obj.macY, ...
                                           zoffset, ...
                                           'x', 'LineWidth', lw, ...
                                           'Color', colors.red);
        ...
        function createTiles(obj)
            ...
            for i = 1:obj.N(1)
                for j = 1:obj.N(2)
                    ...
                    % store control points of positive y half of wing
                    obj.controlPointsX(i,j) = obj.tiles{i,j}.controlPoint.x;
                    obj.controlPointsY(i,j) = obj.tiles{i,j}.controlPoint.y;
                    
                    % store tile horseshoe midpoints
                    obj.horseshoeMidpointsX(i,j) = ...
                        obj.tiles{i,j}.horseshoe.middle(1);
                    obj.horseshoeMidpointsY(i,j) = ...
                        obj.tiles{i,j}.horseshoe.middle(2);
                    obj.horseshoeMidpointsX(i,j+yTilesPerWing) = ...
                        obj.tiles{i,j+yTilesPerWing}.horseshoe.middle(1);
                    obj.horseshoeMidpointsY(i,j+yTilesPerWing) = ...
                        obj.tiles{i,j+yTilesPerWing}.horseshoe.middle(2);
                    
                    % store tile spans
                    obj.tileSpans(i,j) = ...
                        obj.tiles{i,j}.horseshoe.right(2) - ...
                        obj.tiles{i,j}.horseshoe.left(2);
                    obj.tileSpans(i,j+yTilesPerWing) = ...
                        obj.tiles{i,j+yTilesPerWing}.horseshoe.right(2) - ...
                        obj.tiles{i,j+yTilesPerWing}.horseshoe.left(2);
                end
            end
        end
    end    
